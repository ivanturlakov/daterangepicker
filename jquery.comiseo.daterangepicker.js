...
(function($, window, undefined) {
	...
	function buildCalendar(classnameContext, options) {
		...
		function onSelectDay(dateText, instance) {
            let rangePreview = document.querySelector('.range-preview');
            ...
            if(range) {
                let formattedRange = formatRangeForDisplay(range);
                rangePreview.innerHTML = `${formattedRange}`
            }

            // Confirm button state
            let confirmButton = $('.apply-button');
            let searchDatesMsg = $('.search-dates-msg');
            ...
            // Catch start date
            setTimeout(() => {
                let rangeList = document.querySelectorAll('td.ui-state-highlight');
                rangeList[0].classList.add('start-date-link');
                if(range.end) rangeList[rangeList.length - 1].classList.add('end-date-link');
            }, 50)
		}
	...
	function buildButtonPanel(classnameContext, options, handlers) {
		...
		function init() {
			...
			if (options.applyButtonText) {
				applyButton = $('<button type="button" class="ui-priority-primary apply-button"></button>')
